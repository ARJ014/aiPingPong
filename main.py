...
# We are training an ai against an ai and take the total sum of hits as the fitness         
def eval_genomes(genome,config):
    width,height=700,500
    window=pygame.display.set_mode((width,height))
    
    for i, (genome_id1,genome1) in enumerate(genome):
        if i==len(genome)-1: break
        genome1.fitness=0
        for (genome_id2,genome2) in genome[i+1:]:
...
def test_ai(config):
    ...
    game.test_ai(winner,config=config)
